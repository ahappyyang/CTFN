...
def sampling(dataset, ground_truth,CLASSES_NUM):
    if dataset == 'Indian':
        proportion=0.015
    elif dataset == 'PaviaU':
        proportion=0.002
    ...
    for i in range(CLASSES_NUM):
        ...
        if proportion != 1:
            # nb_val = max(int(proportion * len(indexes)), 1)
...
def get_data(dataset):
    ...
    CLASSES_NUM = max(gt)
    train_indices, test_indices, train_num,test_num = sampling(dataset, gt_hsi, CLASSES_NUM)
    _, total_indices,_,total_num = sampling('all', gt_hsi , CLASSES_NUM)
    ...
    return  data_hsi,CLASSES_NUM,train_indices,test_indices,total_indices,y_train, y_test, y_true, gt,y_train_flatten,y_test_flatten
